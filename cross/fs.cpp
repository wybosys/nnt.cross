...
{
    bool r = SetCurrentDirectoryA(str.c_str());
	if (!r)
...
{
    auto r = ::rename(from.c_str(), to.c_str()) == 0;
    if (!r)
        Logger::Warn("重命名失败 " + from + " to " + to);
    return r;
}
...
#ifdef NNT_WINDOWS
        char path[MAX_PATH];
        ::SHGetFolderPathA(NULL, CSIDL_COMMON_DOCUMENTS, NULL, 0, path);
        gs_path_home = path;
#else
		gs_path_docs = getenv("HOME");
#endif
	}
	return gs_path_docs;
}
...
string path_tmp()
{
	if (gs_path_tmp.empty())
	{
        ...
        char *buf;
        size_t lbuf;
        _dupenv_s(&buf, &lbuf, "TMP");
        gs_path_tmp = buf;
        free(buf);
#else
		gs_path_tmp = getenv("TMP");
#endif
	}
	return gs_path_tmp;
}
